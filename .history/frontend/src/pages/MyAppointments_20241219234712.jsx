        ...
        {appointments.length > 0 ? (
          <div className="flex flex-col gap-6">
            {appointments.map((appointment) => (
              <div
                key={appointment._id}
                className="flex items-center bg-gray-800 rounded-lg shadow-lg hover:bg-gray-900 transition-all duration-300 p-6"
              >
                {/* Doctor's Image */}
                <img
                  src={appointment.docData?.image}
                  alt={`Dr. ${appointment.docData?.name}`}
                  className="w-32 h-32 rounded-lg object-cover mr-6 border-2 border-gray-700 shadow-lg"
                />
