...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (i === 0) {
        // Adjust to the next 30-minute interval for today
        const nextAvailableMinutes =
          currentDate.getMinutes() + (30 - (currentDate.getMinutes() % 30));
        currentDate.setMinutes(nextAvailableMinutes);

        // If adjusted time exceeds 8:30 PM, skip slot generation for today
        if (
      ...
      <div className="mt-8">
        ...
        {selectedSlot && (
          <div className="mt-4 text-white">
            ...
            <p className="border border-blue-300 w-32 rounded-lg  p-5 text-center my-10">
              {selectedSlot.time}
            </p>
          </div>
        )}
