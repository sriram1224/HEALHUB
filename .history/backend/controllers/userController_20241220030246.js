...
const verifyRazor = async (req, res) => {
  try {
    ...
    if (orderInfo.status === "paid") {
      // Find the appointment using the order_id (stored in receipt or payment-related field)
      const appointment = await appointmentModel.findOne({
        "receipt.orderId": razorpay_order_id,
      });

      if (!appointment) {
        return res.status(404).json({
          success: false,
          message: "Appointment not found for the order",
        });
      }

      // Update the payment status to true
      const updatedAppointment = await appointmentModel.findByIdAndUpdate(
        appointment._id, // Use _id to identify the document
        { payment: true } // Get the updated document
      );
