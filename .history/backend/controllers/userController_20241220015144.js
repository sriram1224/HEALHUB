...
export const cancelAppointment = async (req, res) => {
  ...
  try {
    ...
    console.log("Doctor Data:", doctorData); // Log doctor data

    let slots_booked = doctorData.slots_booked || {};
    const slotDateKey = slotDate.toISOString().split("T")[0]; // Format date as string for key

    console.log("Doctor's Booked Slots:", slots_booked); // Log slots for that date

    if (slots_booked[slotDateKey]) {
      // Log current slots for the date
      console.log("Current Slots on the Date:", slots_booked[slotDateKey]);
...
const razorpayInstance = new razorpay({
  key_id: "rzp_test_HVzUoMU4jcFJUK",
  key_secret: "m2EAEEinu0TKHHy0dtCNisdt",
});
...
export const createRazorpayOrder = async (req, res) => {
  try {
    ...
    const options = {
      amount: Math.round(appointment.amount * 100), // amount in smallest currency unit
      currency: "INR",
      receipt: `rcpt_${appointmentId}`,
      payment_capture: 1,
      notes: {
        appointmentId: appointmentId,
        patientName: req.user.name, // Assuming user info is attached to req
...
export const validateRazorpayRequest = (req, res, next) => {
